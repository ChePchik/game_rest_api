...
// Миддлвар для проверки конца игры
function GameEnd(req, res, next) {
	// Проверяем, существует ли req.user
	if (!req.user) {
	...
	// Если пользователя нет в списке, возвращаем ошибку
	if (!user) {
...
app.get("/interact", authenticateToken, checkProgress(4), (req, res) => {
	...
	if (id && id == 1) {
		res.json({
			...
			mystery:
				"Я живу внутри машины, Из нулей и единиц я сшит. Решаю задачи без заминок, А если ошибка — я молчит.",
		});
		user.progress++;
		saveUsers(users);
	} else {
